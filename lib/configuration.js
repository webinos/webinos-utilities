...
function Config(webinosType, inputConfig) {
    ...
    */
    function setWebinosMetaData() {
        ...
        ConfigContext.fileList = [{folderName: undefined, fileName: "metaData", object: ConfigContext.metaData},
            {folderName: undefined, fileName:"trustedList", object: ConfigContext.trustedList},
            {folderName: undefined, fileName:"untrustedList", object: ConfigContext.untrustedCert},
            {folderName: undefined, fileName:"exCertList", object: ConfigContext.exCertList},
    ...
    this.loadCertificates = function(cert) {
        ...
        var certList = [
            {folderName: undefined, fileName:"crl", object: ConfigContext.cert.crl},
    ...
    this.fetchDetails = function(folderName, fileName, assignValue) {
        ...
        try {
            ...
            if (dataString !== "") {
                ...
                ConfigContext.emit("READ_ERROR", new Error("Webinos configuration data is empty in the file, considering it is corrupted -"+ err));
                return false;
            }
        } catch(error) {
            ConfigContext.emit("EXCEPTION", new Error("webinos configuration data is corrupted, webinos will not start correctly -"+ err));
