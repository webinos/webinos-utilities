              ...
              stat.isDirectory()) {
                ...
                if (present === false) {
                    ...
                    try {
                        ...
                        testFile = fs.readdirSync(path.resolve(node_moduleDir, fileList[i], "test", "web_root"));
                        for (k = 0 ; k < testFile.length; k = k + 1){
                            readFile = fs.readFileSync( path.resolve(node_moduleDir, fileList[i], "test", "web_root", testFile[k]));
                            fs.writeFileSync(path.resolve(node_moduleDir, "../", "web_root", "testbed", testFile[k]), readFile.toString());
                            if (path.resolve(node_moduleDir, "../", "web_root", "testbed", testFile[k]).search(".html") !== -1)
                    ...
                    try {
                        // Copy the unit tests..
                        testFile = fs.readdirSync(path.resolve(node_moduleDir, fileList[i], "test", "jasmine"));
                        for (k = 0 ; k < testFile.length; k = k + 1){
                            readFile = fs.readFileSync(path.resolve(node_moduleDir, fileList[i], "test", "jasmine", testFile[k]));
                            fs.writeFileSync(path.resolve(node_moduleDir, "../", "web_root","test", testFile[k]), readFile.toString());
                            if (path.resolve(node_moduleDir, "../", "web_root", "test", testFile[k]).search(".html") !== -1)
