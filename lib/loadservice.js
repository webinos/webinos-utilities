...
exports.checkForWebinosModules = function(node_moduleDir) {
    try {
        ...
        var fileList = fs.readdirSync(node_moduleDir), name, stat, config = {"instancesParams": []}, i, modules = [];
            ...
            if (fileList[i].indexOf("webinos-api") !== -1 && stat.isDirectory()) {
                ...
                if (!existsSync(customisedConfigFile)) {
                    try{
                        ...
                        fileList[i].split(path.sep).map(function(folderFragment){
                            tmpFolder = path.join(tmpFolder, folderFragment);
                            if(!existsSync(tmpFolder)) {
                                fs.mkdirSync(tmpFolder);
                            }
                        });
                        fs.writeFileSync(customisedConfigFile, JSON.stringify(config, null, "  "));
                    } catch (err) {}
                } else {
                    try{
