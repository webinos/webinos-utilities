...
function copyTestPage(apiName, name) {
    try {
        ...
        if (stat.isDirectory()){
            if(!fs.existsSync(path.join(node_moduleDir, "../", "web_root", "testbed",apiName, name))){
            ...
            for (k = 0 ; k < testFile.length; k = k + 1){
                ...
                if (stat.isDirectory()){
                    copyTestPage(apiName, path.join(name, testFile[k]));
                } else {
...
function removeTestPages(modules) {
...
exports.checkForWebinosModules = function() {
    try {
        ...
        for (i = 0; i < fileList.length; i = i + 1) {
            ...
            if (fileList[i].indexOf("webinos-api") !== -1 &&
                    fileList[i].indexOf("webinos-api-service") === -1 &&
                    stat.isDirectory()) {
                try {
                ...
                testPath = copyTestPage(fileList[i]);
                unitTest="";
                try {
                    // Copy the unit tests..
                ...
                modules.push({name: fileList[i], params: config.params, testFile: testPath, unitTestFile: unitTest});
...
exports.createWebinosJS = function() {
    ...
    data = fs.readFileSync(path.join(node_moduleDir,"webinos-api-serviceDiscovery","wrt","webinos.servicedisco.js"));
    webinosJS.write(data.toString());
};
...
exports.addWebinosJS = function() {
    var path = require("path");
    var fs = require("fs");
    var wrt_Dir, fileList, data, i, j, fileName, webroot_Dir;
    webroot_Dir = path.join(__dirname, "../../../web_root");
