...
function load(mod, modDesc, registry, rpcHandler, config, moduleHttpHandlers) {
    try {
        if (mod.Module) {
            ...
            modDesc.instancesParams.map(function(params){
        ...
        for (i = 0; i < fileList.length; i = i + 1) {
            ...
            if (fileList[i].indexOf("webinos-api") !== -1 && stat.isDirectory()) {
                try {
                    ...
                    var serviceCacheFile = path.join(require("./webinosPath.js").webinosPath(), "userData", "serviceCache.json");
                    if (existsSync(serviceCacheFile)){
                        var serviceCache = require(serviceCacheFile);
                        //TODO: add service address constraints
                        config.instancesParams = serviceCache
                            .filter(function(service){return (service.api == apiURI)})
                            .map(function(service){return service.params});
                    }
                    if (!config.instancesParams){
                        if (config.params){
                            config.instancesParams = [config.params];
                        } else {
                            config.instancesParams = [{}];
                        }
                    }
                ...
                modules.push({
                    name: name,
                    instancesParams: config.instancesParams,
                    path: apiModDir,
                    testFile: testPath
                });
                config.instancesParams.map(function(params) {
                    logger.log("New module "+fileList[i]+" detected with configuration parameters "+ require("util").inspect(params) + " and testPath "+testPath);
                });
            }
        }
        return modules;
