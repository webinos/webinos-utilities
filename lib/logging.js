...
var writeInfo = {}, writeError = {};
 ...
 function WebinosLogging (filename) {
    ...
    var cyan = '\u001b[36m';
    var yellow = '\u001b[33m';
    var logDebugMsg, logErrMsg;

    if (os.platform() === 'android') {
        var l = function(prev, a) {
            return prev + ' ' + a;
        };
        var dbg = require('debuglog.node');
        logDebugMsg = console.log = function() {
            dbg.log(Array.prototype.slice.call(arguments).reduce(l, ''));
        };
        logErrMsg = console.error = function() {
            dbg.error(Array.prototype.slice.call(arguments).reduce(l, ''));
        };
    } else {
        logDebugMsg = util.print;
        logErrMsg = util.error;
    }
    ...
    logging.error = function (msg) {
        ...
        var formattedMsg = yellow + "[" + getTime() + "]"+red+" error " + green + name + "(" + getLineNumber () + ")" + id + red + msg + "\n";
    ...
    logging.log = function (msg) {
        ...
        if (os.type ().toLowerCase () === "windows_nt") { name = this.name.split ("\\").pop (); } else { name = this.name.split ("/").pop ();}
        var formattedMsg = yellow + "[" + getTime() + "]"+ blue+" info " + green + name + "(" + getLineNumber () + ")" + id + cyan + msg + "\n";
