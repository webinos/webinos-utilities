...
(function () {
	...
	var Discovery = function(rpcHandler, params) {
		...
		this.findConfigurableAPIs = function (params, successCB, errorCB, objectRef) {
            var modulesBasePath = require.main.paths[0];
            var modulesFolders = fs.readdirSync(modulesBasePath);
            var installedAPIs = [];

            for (var i in modulesFolders) {
                var moduleBasePath = path.join(modulesBasePath, modulesFolders[i]);
                var configFilePath = path.join(moduleBasePath, "config.json");
                var templateFilePath = path.join(moduleBasePath, "template.json");
                var packageFilePath = path.join(moduleBasePath, "package.json");

                if (fs.existsSync(configFilePath) && fs.existsSync(packageFilePath) && fs.existsSync(templateFilePath) ) {
                    if (require(configFilePath).params) {
                        installedAPIs.push({
                            'name': require(packageFilePath).name
                          , 'path': moduleBasePath
                        });
                    }
                }
