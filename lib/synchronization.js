...
//                        contentsPzp[key] = jsonObject[key];
    ...
    function isLocalAddress(str, ownId){
        if (Object.prototype.toString.call(str) === "[object Object]") {
            ...
            if (str.split('/')[0] === ownId.split('/')[0]) { // only compare pzh part of the id
                return true;
            }
        ...
        if (Object.prototype.toString.call(remoteJson) === "[object Array]" &&
            ...
            remoteJson.forEach(function(rname, rindex){
                if (rname /*&& rname.id*/) {
                    ...
                    localJson.forEach(function(lname, index){
                       ...
                       if (index+1 === localJson.length && !found) {
                           if (rname.serviceAddress){
                               if(!contains(localArr, rname) && rname.serviceAddress !== ownId)
                                   localArr.push(rname);
                           } else {
                           if(!contains(localArr, rname)) localArr.push(rname);
                           }
                           if(!contains(localArr, lname) && isLocalAddress(lname, ownId)) localArr.push(lname);
                      }
                    });
                ...
                if (rindex+1 === remoteJson.length) {
                    localJson.forEach(function(lname, lindex){
                       if(!contains(localArr, lname) && isLocalAddress(lname, ownId))
                            localArr.push(lname);
                    });
                }
            });
            ...
            if (localJson && typeof localJson === "object") {
                for (key in localJson) {
                    if (!remoteJson.hasOwnProperty(key) && isLocalAddress(key, ownId)){ // ignore remoteJSON as above part should handle it
